...
def test_format_alert_message_fatal():
    msg = format_alert_message(("fatal", "pipeline-v2", "timeout"))
    assert "Pipeline 失败" in msg
    assert "pipeline-v2" in msg
...
def test_send_feishu_alerts_success(mock_post):
    ...
    alerts = [("fatal", "pipeline-v2", "timeout")]
...
def test_send_feishu_alerts_no_webhook_returns_false(mock_post):
    ...
    with patch.dict(os.environ, {}, clear=True):
        ok = send_feishu_alerts([("fatal", "pipeline-v2", "y")], webhook_url=None)
        assert ok is False
