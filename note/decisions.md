...
> **当前状态：** 下方已有条目主要记录 V1 的历史决策，不代表 V2 仍沿用。V2 已决定作为新数据系统直接建设，不迁移旧 SQLite、不兼容旧 `prices.json`，当前架构基准见 [docs/data-pipeline.md](../docs/data-pipeline.md)。历史条目保留是为了说明项目曾经如何演进。
